...
## SM4(br)
```
reduceExposure.py /work/drp --calib=$CALIB --rerun=$RERUN --longlog=1 -j 16 --longlog 1 --id visit=104149 spectrograph=4 arm=r --config isr.doFlat=True adjustDetectorMap.doSlitOffsets=False --clobber-config --no-versions 2>&1 | tee -a $LOG/extractionQA/reduceExposure.log
